...
def bi_gated_self_attention(
        rep_tensor, rep_mask, scope=None, keep_prob=1., is_train=None, wd=0.,
        activation='elu', hn=None):
    with tf.variable_scope(scope or 'bi_gated_self_attention'):
        fw_rep = gated_self_attention(
            rep_tensor, rep_mask, 'gated_self_attention_fw', keep_prob, is_train, wd,
            activation, hn, position_mask_type='forward'
        )
        bw_rep = gated_self_attention(
            rep_tensor, rep_mask, 'gated_self_attention_bw', keep_prob, is_train, wd,
            activation, hn, position_mask_type='backward'
        )
        return tf.concat([fw_rep, bw_rep], -1)


def gated_self_attention(
        rep_tensor, rep_mask, scope=None, keep_prob=1., is_train=None, wd=0.,
        activation='elu', hn=None, position_mask_type=None):
    bs, sl, vec = tf.shape(rep_tensor)[0], tf.shape(rep_tensor)[1], tf.shape(rep_tensor)[2]
    ivec = rep_tensor.get_shape().as_list()[2]
    ivec = hn or ivec
    with tf.variable_scope(scope or 'gated_self_attention_%s' % (position_mask_type or 'None')):
        rep_map = bn_dense_layer(rep_tensor, ivec, True, 0., 'bn_dense_map', activation,
                                 False, wd, keep_prob, is_train)
        ...
        if position_mask_type in ['forward', 'backward']:
            ...
            position_mask = tf.tile(tf.expand_dims(position_mask, 0), [bs, 1, 1])
        ...
        with tf.variable_scope('intra_sent_attn'):  # bs,sl,hn
            ...
            pre_align_score = bn_dense_layer(  # bs,sl,hn
                rep_tensor_for_attn, ivec, True, 0., 'intra_sent_map1', activation, False, wd, keep_prob, is_train)
            align_score = bn_dense_layer(  # bs,sl,hn
                pre_align_score, ivec, True, 0., 'intra_sent_map2', 'linear', False, wd, keep_prob, is_train)
            align_score_w_mask = exp_mask_for_high_rank(align_score, rep_mask)  # bs,sl,hn
        ...
        with tf.variable_scope('context_fusion_gate'):
            fusion_gate = tf.nn.sigmoid(bn_dense_layer(
                [rep_map, attn_res], hn, True, 0., 'linear_fusion_gate', activation, False, wd, keep_prob, is_train
            ))
