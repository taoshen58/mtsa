...
class ModelTemplate(metaclass=ABCMeta):
    ...
    def build_loss(self):
        ...
        self.xentropy_loss = tf.reduce_mean(losses, name='xentropy_loss')
        tf.add_to_collection('losses', self.xentropy_loss)
    ...
    def update_tensor_add_ema_and_opt(self):
        ...
        # add gradient clip
        # train_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, self.scope)
        # gradients = tf.gradients(self.loss, train_vars)
        # # # clip the gradients
        # grads, global_norm = tf.clip_by_global_norm(gradients, 2.)
        # # # define train op
        # self.train_op = self.opt.apply_gradients(
        #     zip(grads, train_vars), global_step=self.global_step, name='train_step'
        # )

        self.train_op = self.opt.minimize(
            self.loss, self.global_step, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, self.scope))
    ...
    def step(self, sess, batch_samples, get_summary=False):
        ...
        if get_summary:
            loss, summary, train_op = sess.run(
                [self.xentropy_loss, self.summary, self.train_op], feed_dict=feed_dict)

        else:
            loss, train_op = sess.run([self.xentropy_loss, self.train_op], feed_dict=feed_dict)
            summary = None
